...
var beamBlockConfig = require('./libs/beam-blockconf');
...
try {
  require('newrelic');
  if (cluster.isMaster) logger.debug('NewRelic', 'Monitor', 'New Relic initiated');
} catch (e) {}
      ...
      for (var g = 0; g < portsF.length; g++) {
        if (ports.indexOf(portsF[g]) !== -1) {
          logger.error(
            ...
            'Has same configured port of ' + portsF[g] + ' as ' + poolConfigFiles[i].fileName,
          );
          process.exit(1);
          return;
        }
    ...
    if (!Array.isArray(pcfg.daemons) || pcfg.daemons.length < 1) {
      ...
      connection.on('ready', function() {
        logger.debug(
          'PPLNT',
          coin,
          'TimeShare processing setup with redis (' + redisConfig.host + ':' + redisConfig.port + ')',
        );
      });
    }
  });
  ...
  var spawnInterval = setInterval(function() {
    ...
    if (i === numForks) {
      ...
      logger.debug(
        'Master',
        'PoolSpawner',
        'Spawned ' + Object.keys(poolConfigs).length + ' pool(s) on ' + numForks + ' thread(s)',
      );
    }
  }, 250);
};
...
module.exports = function(logger, poolConfig) {
  ...
  logger.debug('DEBUG', 'DEBUG', '********* mposConfig=' + JSON.stringify(mposConfig));
